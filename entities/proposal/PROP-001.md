...
That works, but the guarantee is soft: whether a child schema actually conforms to the contract is checked by review, not by the engine. This is exactly the kind of integrity [hard integrity must not depend on model discipline (REQ-004)](../requirement/REQ-004.md) says should not depend on model discipline.
...
- Stays fully declarative — schemas remain data, per [the validation engine is generic and stable; schemas are data (DEC-002)](../decision/DEC-002.md). No new engine behavior at record-validation time beyond the merged field set.
