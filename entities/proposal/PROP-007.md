...
Split from the last open question of [raw-text scan: pre-parse checks for what the parse destroys (PROP-006)](PROP-006.md): the Markdown body of a record has its own version of the silent-misread problem — a ref malformed enough that the resolver does not recognize it does not error, it simply reads as plain prose. Should the scan's jurisdiction extend there? The question deserves an address, so it is registered; it is also rejected on arrival, and the status says so.
...
Rejected is not sealed. The membership discipline of [PROP-006](PROP-006.md) applies here too: this judgment stands until a silent Markdown misread is *measured* doing real damage in use — a record whose meaning changed and nobody saw it. That evidence would reopen the question with a concrete member to seed from, which is the only way rules enter the scan anyway.
