...
Rationale: if the agent regenerated a bespoke linter on every redesign, each regeneration would be an opportunity for silent bugs — and a bug in the validator is worse than a bug in the data, because it corrupts trust in the whole system. A fixed, tested engine makes schema evolution cheap ([schema evolution is a first-class workflow](../requirement/REQ-005.md)^[REQ-005](../requirement/REQ-005.md)) and keeps hard integrity deterministic ([hard integrity must not depend on model discipline](../requirement/REQ-004.md)^[REQ-004](../requirement/REQ-004.md)).
