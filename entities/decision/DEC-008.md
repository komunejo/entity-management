...
Origin: iteration-2 evaluation. The iteration-1 repair agent demoted the ghost mentor and the dangling tool link to prose (green); the iteration-2 agent left both red awaiting a human. Both were defensible because [per-project contract policy, asked explicitly at first-schema creation (DEC-007)](DEC-007.md) had not defined the boundary; this decision defines it.
