...
After a skill upgrade, read the CHANGELOG delta before working: contract changes are never silent, and the changelog is where they speak to you — what went green, what can newly go red, and what is now expected of you. A green `validate` from there confirms where your project stands.

## Workflows

### Initialize a project

Adopting this means a structure gets **declared**: an explicit schema, authored and owned. The engine infers nothing — it validates against the declaration and against nothing else, so until one exists there is nothing for a record to be right or wrong about. That is the misread this skill sees most from the field, and it is worth naming when the user's expectation runs the other way (a capable adopter staged 240 unprepared notes, counted 171 errors, and read the count as a verdict against the method; those errors measured a corpus against a schema nobody had authored).

Where the declaration comes from is open. Reading the corpus and proposing a shape from what is already there is good work, often the fastest route, and squarely yours when the user's standing arrangement gives you that latitude — as is how much of it you settle alone and how much you bring back. One exception is not a matter of latitude but of possibility: against an uncontracted source that still moves under you, a shape read off the data grants confidence without foundation, because no error is decidable (is the record wrong, or did the format shift?); there, convert into a structure the owner controls rather than validate one nobody wrote. Adoption also costs a revision once real records meet the schema — state that cost plainly rather than discovering it together at error 171.

It is a legitimate answer that a corpus should **not** be adopted: nothing will reference or validate it, there is no recurring kind, or it is still finding its shape (`strict: false` while it does). Partial adoption — one type, the one that hurts — is a normal starting point, not a compromise. This same conversation is written down for the user to read, in the skill's repository under `docs/` (address in Bundled resources) — point them there when they would rather read it than be told it.
...
3. **When you choose the filename, keep parentheses out of it** — and angle-bracket any destination that has them. A reference whose destination holds unwrapped parentheses is not seen by the engine at all: not resolved, not reported (ISSUE-017, open). So `THG-001-draft.md` rather than `THG-001 (draft).md`, and `[label (ID)](<some note (draft).md>)` whenever the target's name already carries them. Two limits on this: it applies to names *you* pick — never rename a project's existing files to work around an engine limitation, and never do it silently; and in a `filename: free` project the names belong to the project, so if its corpus is full of parenthesized names, say so and write every reference angle-bracketed rather than proposing a mass rename.
4. Run `validate`.
...
- [`references/troubleshooting.md`](references/troubleshooting.md) — flag triage: who owns each error family, when to ask the human (once, about intent, never mechanics), and recording the criteria their answers establish. Its human-facing companions do not travel with the skill: they live in the skill's repository, **github.com/komunejo/entity-management**, under `docs/` — every engine message explained with examples, and the guide to what adopting asks of an owner. Give a human that address; a `docs/` path is not a path that exists in the project you are working in.
- [`references/comment-decidability.md`](references/comment-decidability.md) — how the scan treats YAML comments, what it knows from the schema (`pattern` included), and your three routes when a comment flag fires. Read on the first comment flag, or before proposing a `pattern` declaration.
